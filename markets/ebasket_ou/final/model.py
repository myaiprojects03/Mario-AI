"""
eBasketball Over/Under Model Training & Walk-Forward Validation Pipeline (Task 12).

Usage:
    python -m markets.ebasket_ou.model

Overview:
1. Loads raw historical matches strictly WHERE source = 'csv_backfill' AND sport = 'ebasket'.
2. Executes Task 7 eBasketball O/U feature engineering pipeline (EBASKET_OU_FEATURE_COLUMNS).
...
5. Enforces 1.60 minimum odds floor for O/U markets (BacktestEngine(global_odds_floor=1.60)).
6. Compares Normal Baseline vs Ensemble Strategy C (P >= 0.50) vs Ensemble Strategy D (Value Edge).
7. Calculates empirical t-test, p-value, and 95% Confidence Interval on total net units.
8. Exports model artifact to markets/ebasket_ou/artifacts/ and writes RESULTS.md report.
"""
...
from markets.ebasket_ou.features import (
    build_ebasket_ou_features,
...
logger = logging.getLogger("markets.ebasket_ou.model")

MODEL_VERSION = "v1.0.0"
EBASKET_OU_ODDS_FLOOR = 1.60  # Explicit 1.60 minimum odds floor for O/U markets
...
def load_training_dataset(engine) -> pl.DataFrame:
    ...
    SELECT DISTINCT ON (m.match_id)
        ...
        CAST(o.line_value AS FLOAT) AS "ou_line_value",
    ...
    logger.info("Executing Task 7 eBasketball O/U feature engineering pipeline...")
    df_features_sub = build_ebasket_ou_features(df_raw)

    meta_cols = ["match_id"] + [
        c for c in ["startedAt", "match_start_time", "home.goals", "away.goals",
                    "odds.over_under.line", "closingOdds.over_under.over", "odds_close"]
    ...
    logger.info(f"Final eBasketball O/U dataset row count after feature pipeline exclusions: {final_count:,} matches.")
    assert 18000 <= final_count <= 30000, f"Expected eBasketball row count in ~18,000-30,000 range, got {final_count:,}"
...
def run_walk_forward_backtest(df: pl.DataFrame) -> Dict[str, Any]:
    """
    Executes walk-forward evaluation across identical time splits with inner holdout calibration and 1.60 odds floor.
    """
    ...
    ensemble_model = LightGBMNNEnsembleOUModel(weight_lgb=0.60)

    norm_preds, ens_preds = [], []
    test_records = []

    logger.info(f"Starting walk-forward validation splits (Odds Floor: {EBASKET_OU_ODDS_FLOOR})...")
    split_count = 0
    ...
    for train_idx, test_idx in splitter.split(df):
        ...
        ens_sub = LightGBMNNEnsembleOUModel(weight_lgb=0.60)
        ens_sub.fit(df_tr_sub)
        p_val_sub = ens_sub.predict_probs(df_val_sub)

        df_val_pd = df_val_sub.to_pandas()
        line_col_val = "odds.over_under.line" if "odds.over_under.line" in df_val_pd.columns else "ou_line_value"
        ...
        line_col = "odds.over_under.line" if "odds.over_under.line" in df_test_pd.columns else "ou_line_value"
        ...
        for idx in range(len(df_test_pd)):
            ...
            norm_preds.append(p_norm[idx])
            ens_preds.append(p_ens[idx])

    logger.info(f"Walk-forward splits completed: {split_count} folds evaluated.")

    # Convert test records to Polars dataframe for evaluation using 1.60 odds floor
    df_eval_base = pl.DataFrame(test_records)
    engine = BacktestEngine(global_odds_floor=EBASKET_OU_ODDS_FLOOR)

    # Evaluate Normal Baseline (Raw P >= 0.50)
    df_norm = df_eval_base.with_columns(pl.Series("confidence", norm_preds))
    res_norm = engine.evaluate_tips(df_norm, is_money_line=False)

    # Evaluate Ensemble Strategy C (Holdout Calibrated P >= 0.50)
    df_ens_c = df_eval_base.with_columns(pl.Series("confidence", ens_preds))
    res_ens_c = engine.evaluate_tips(df_ens_c, is_money_line=False)
    ...
    engine_unfiltered = BacktestEngine(global_odds_floor=EBASKET_OU_ODDS_FLOOR, min_confidence=0.0)
    res_ens_d = engine_unfiltered.evaluate_tips(df_ens_d, is_money_line=False)

    print("\n==========================================================================")
    print(f"   AUDITABLE MODEL SELECTION COMPARISON (eBasket O/U Floor: {EBASKET_OU_ODDS_FLOOR})")
    print("==========================================================================")
    print(f"1. Normal Baseline:           {res_norm['total_units']:+,.2f} Units | ROI: {res_norm['roi_pct']:+.2f}% | Hit Rate: {res_norm['hit_rate']*100:.2f}% | Tips: {res_norm['tips_evaluated']:,}")
...
def generate_model_scores_df(
    ...
    recommended_lines = eval_pd[line_col].astype(float).tolist() if line_col in eval_pd.columns else [111.5] * len(eval_pd)
    ...
    model_scores = pl.DataFrame({
        ...
        "model_version": [f"ebasket_ou_{MODEL_VERSION}"] * len(eval_pd),
...
def save_artifact_and_reports(
    ...
    artifact_dir: str = "markets/ebasket_ou/artifacts",
):
    """
    ...
    md_content = f"""# eBasketball Over/Under Model Results & Walk-Forward Backtest Report

> [!CAUTION]
> **NO MODEL IS RECOMMENDED FOR LIVE DEPLOYMENT AT THIS TIME.**
> None of the evaluated candidate strategies cleared positive expected value against the {EBASKET_OU_ODDS_FLOOR:.2f} odds floor on clean historical data ({bt['total_units']:+,.2f} Units, {bt['roi_pct']:+.2f}% ROI). No model or strategy is selected as a "winner" for this market, as doing so would misleadingly imply production readiness.

## Executive Summary
- **Model Version**: `{MODEL_VERSION}`
- **Deployment Status**: 🔴 **REJECTED FOR LIVE DEPLOYMENT** (Negative net expected value)
- **Market Odds Floor**: **{EBASKET_OU_ODDS_FLOOR:.2f} Minimum Odds Floor** (Global floor applied per spec)
- **Total Training Matches Used**: **{total_training_rows:,} matches** (`source = 'csv_backfill'`)
- **Best Candidate Strategy**: `Ensemble Value-Edge Strategy D (LightGBM + MLP NN)` ({bt['total_units']:+,.2f} Net Units)
...
| **2-Sided p-value** | **{sa['p_val_2sided']:.4f}** | ⚠️ **INCONCLUSIVE ($p > 0.05$)** |
| **95% Confidence Interval (Total Units)** | **[{sa['ci_units'][0]:+,.2f}, {sa['ci_units'][1]:+,.2f}] Units** | 95% total yield range |
| **95% Confidence Interval (ROI %)** | **[{sa['ci_roi'][0]:+.2f}%, {sa['ci_roi'][1]:+.2f}%]** | 95% ROI range |

> [!NOTE]
> **STATISTICAL INTERPRETATION & FORWARD PATH**:
> The eBasketball O/U backtest result is **INCONCLUSIVE ($p = {sa['p_val_2sided']:.4f}$)**. While Strategy D significantly reduces losses compared to Strategy C, we cannot confirm or rule out an edge with the current sample size ($N = {sa['n_tips']:,}$).
>
...
|---|---|---|---|
| **Total Net Units** | {res_c['total_units']:+,.2f} Units | **{res_d['total_units']:+,.2f} Units** | Strategy D reduces losses by {res_d['total_units'] - res_c['total_units']:+,.2f} Net Units |
| **Overall ROI (%)** | {res_c['roi_pct']:+.2f}% | **{res_d['roi_pct']:+.2f}%** | Strategy D improves ROI by {res_d['roi_pct'] - res_c['roi_pct']:+.2f}% |
| **Hit Rate (%)** | {res_c['hit_rate']*100:.2f}% | **{res_d['hit_rate']*100:.2f}%** | Strategy D achieves higher hit rate |
...
## 2. Auditable Model Architecture Comparison (Walk-Forward Backtest with {EBASKET_OU_ODDS_FLOOR:.2f} Odds Floor)

| Model Architecture / Strategy | Net Units Generated | ROI (%) | Hit Rate (%) | Tips Evaluated | Deployment Status |
|---|---|---|---|---|---|
| **Normal Distribution Baseline** | {sbs['normal_baseline']['total_units']:+,.2f} | {sbs['normal_baseline']['roi_pct']:+.2f}% | {sbs['normal_baseline']['hit_rate']*100:.2f}% | {sbs['normal_baseline']['tips_evaluated']:,} | Baseline (Rejected) |
...
| **Ensemble Strategy D (Value Edge)** | **{sbs['ensemble_d']['total_units']:+,.2f}** | **{sbs['ensemble_d']['roi_pct']:+.2f}%** | **{sbs['ensemble_d']['hit_rate']*100:.2f}%** | **{sbs['ensemble_d']['tips_evaluated']:,}** | **Least-Bad Option (REJECTED FOR LIVE DEPLOYMENT)** |
...
4. **Enforced {EBASKET_OU_ODDS_FLOOR:.2f} Odds Floor**: Enforces minimum odds floor per specification.
"""

    report_path = "markets/ebasket_ou/RESULTS.md"
