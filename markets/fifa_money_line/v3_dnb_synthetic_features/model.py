"""
FIFA Money Line V3 Model Pipeline (Draw-No-Bet Synthetic Transforms & Quarter-Kelly Staking).
"""
...
logger = logging.getLogger("markets.fifa_money_line.v3.model")
...
def load_training_dataset(engine) -> pl.DataFrame:
    logger.info("Querying PostgreSQL core.matches for FIFA Money Line DNB dataset...")
    sql = """
    SELECT DISTINCT ON (m.match_id)
        ...
        CAST(od.odds_close AS FLOAT) AS "odds_draw"
    FROM core.matches m
    ...
    LEFT JOIN core.odds od ON m.match_id = od.match_id AND od.market_type = 'fifa_money_line' AND od.side = 'draw'
    ...
    df_features_sub = build_fifa_ml_v3_features(df_raw)
    meta_cols = ["match_id", "startedAt", "match_start_time", "home.goals", "away.goals", "closingOdds.money_line.home", "odds_close", "odds_draw"]
    ...
    logger.info(f"Loaded {len(df_features):,} FIFA Money Line V3 training matches.")
    return df_features
...
def run_v3_backtest(df: pl.DataFrame) -> Tuple[Any, List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    feature_cols = [c for c in df.columns if c not in [
        ...
        "final_home_score", "final_away_score", "closingOdds.money_line.home", "odds_close", "odds_draw",
    ...
    for train_idx, test_idx in splitter.split(df):
        ...
        xgb.fit(X_tr, y_tr)
        # Predict P(Home Win) and P(Away Win)
        p_home_te = xgb.predict_proba(X_te)[:, 1]
        p_away_te = 1.0 - p_home_te

        for idx in range(len(df_te)):
            odds_h = df_te["closingOdds.money_line.home"].iloc[idx] if "closingOdds.money_line.home" in df_te.columns else 2.10
            odds_d = df_te["odds_draw"].iloc[idx] if "odds_draw" in df_te.columns else 3.80
            odds_a = 2.40  # Baseline away odds proxy

            if pd.isna(odds_h) or float(odds_h) <= 1.0:
                odds_h = 2.10
            if pd.isna(odds_d) or float(odds_d) <= 1.0:
                odds_d = 3.80

            odds_h = float(odds_h)
            odds_d = float(odds_d)

            # 1. Home DNB Synthetic Odds
            dnb_h_odds = odds_h * (1.0 - (1.0 / odds_d))
            p_h = float(p_home_te[idx])
            edge_h = p_h - (1.0 / dnb_h_odds)

            if edge_h >= 0.015 and dnb_h_odds >= ODDS_FLOOR:
...
def save_v3_artifacts(model: Any, tier_results: List[Dict[str, Any]], total_rows: int):
    artifact_dir = "markets/fifa_money_line/v3_dnb_synthetic_features/artifacts"
    ...
    md_content = f"""# FIFA Money Line V3 DNB Synthetic Model Results Report
...
"""
    ...
    report_path = "markets/fifa_money_line/v3_dnb_synthetic_features/RESULTS.md"
