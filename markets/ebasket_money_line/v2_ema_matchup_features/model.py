"""
eBasketball Money Line V2 Model Pipeline & Walk-Forward Evaluator.
Saves model artifact to markets/ebasket_money_line/v2/artifacts/ebasket_money_line_model_v2.0.0.joblib
Writes performance report to markets/ebasket_money_line/v2/RESULTS.md
"""
...
from markets.ebasket_money_line.v2_ema_matchup_features.features import build_ebasket_money_line_v2_features
from markets.ebasket_money_line.models import LogisticRegressionMoneyLineModel, LightGBMNNEnsembleMLModel

logger = logging.getLogger("markets.ebasket_money_line.v2.model")
...
def load_training_dataset(engine) -> pl.DataFrame:
    ...
    df_features = build_ebasket_money_line_v2_features(df_raw)
    meta_cols = [c for c in ["match_id", "startedAt", "home_player", "away_player", "home.goals", "away.goals", "odds_close", "closingOdds.money_line.home"] if c in df_raw.columns]
    if meta_cols:
...
def run_walk_forward_backtest(df: pl.DataFrame) -> Dict[str, Any]:
    ...
    ensemble_model = LightGBMNNEnsembleMLModel()
    ...
    for train_idx, test_idx in splitter.split(df):
        ...
        ens_sub = LightGBMNNEnsembleMLModel()
    ...
    res_c = engine.evaluate_tips(df_ens_c, is_money_line=True)
    ...
    res_d = engine_unfiltered.evaluate_tips(df_ens_d, is_money_line=True)
...
def save_artifact_and_reports(winning_model: Any, backtest_results: Dict[str, Any], total_training_rows: int, artifact_dir: str = "markets/ebasket_money_line/v2_ema_matchup_features/artifacts"):
    ...
    md_content = f"""# eBasketball Money Line V2 Model Results & Walk-Forward Backtest Report
...
"""
    ...
    report_path = "markets/ebasket_money_line/v2/RESULTS.md"
