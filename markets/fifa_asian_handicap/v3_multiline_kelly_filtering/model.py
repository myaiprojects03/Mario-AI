"""
FIFA Asian Handicap V3 Model Pipeline (Moderate Filtering & Quarter-Kelly Staking).
"""
...
logger = logging.getLogger("markets.fifa_asian_handicap.v3.model")
...
def load_training_dataset(engine) -> pl.DataFrame:
    ...
    df_features_sub = build_fifa_ah_v3_features(df_raw)
    meta_cols = ["match_id", "startedAt", "match_start_time", "home.goals", "away.goals", "closingOdds.asian_handicap.home", "odds_close"]
    ...
    logger.info(f"Loaded {len(df_features):,} FIFA Asian Handicap V3 training matches.")
    return df_features
...
def run_v3_backtest(df: pl.DataFrame) -> Tuple[Any, List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    feature_cols = [c for c in df.columns if c not in [
        ...
        "final_home_score", "final_away_score", "closingOdds.asian_handicap.home", "odds_close",
    ...
    handicap_lines = [-0.25, -0.50, -0.75]
    for line_val in handicap_lines:
        odds_mult = 1.75 if line_val == -0.25 else (1.95 if line_val == -0.50 else 2.20)
        for train_idx, test_idx in splitter.split(df):
            df_tr = df[train_idx].to_pandas()
            df_te = df[test_idx].to_pandas()

            X_tr = df_tr[feature_cols].fillna(0).values
            y_tr = (df_tr["home.goals"].values - df_tr["away.goals"].values > -line_val).astype(int)

            X_te = df_te[feature_cols].fillna(0).values
            y_te = (df_te["home.goals"].values - df_te["away.goals"].values > -line_val).astype(float)
...
def save_v3_artifacts(model: Any, tier_results: List[Dict[str, Any]], total_rows: int):
    artifact_dir = "markets/fifa_asian_handicap/v3_multiline_kelly_filtering/artifacts"
    ...
    md_content = f"""# FIFA Asian Handicap V3 Model Results & Sweet-Spot Filtering Report
...
"""
    ...
    report_path = "markets/fifa_asian_handicap/v3_multiline_kelly_filtering/RESULTS.md"
