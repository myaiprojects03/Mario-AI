...
def dispatch_report(is_midnight: bool, target_date: str = None):
    ...
    report_text = generate_performance_report_text(is_midnight=is_midnight, target_date_str=target_date, channel_key="all")
    
    print("\n" + "=" * 60)
    print(f"       DISPATCHING {report_name.upper()}")
    print("=" * 60)
    print(report_text)
    print("=" * 60 + "\n")
    ...
    print(f"\nDispatched {report_name} to {sent_count}/{len(CHANNEL_MAP)} channels successfully!\n")

def main():
    args = parse_args()
    if args.type in ["partial", "both"]:
        dispatch_report(is_midnight=False, target_date=args.date)
    if args.type in ["midnight", "both"]:
        dispatch_report(is_midnight=True, target_date=args.date)
