...
def load_match_data(engine, sport: str):
    sql = f"""
    SELECT 
...
def main():
    print("==========================================================================")
    print("      VERIFYING FEATURE BUILDER ROW COUNTS FOR SOURCE='csv_backfill'")
    print("==========================================================================")
    print(f"Target Database URL: {settings.DATABASE_URL}\n")

    engine = create_engine(settings.DATABASE_URL)

    # 1. FIFA Matches Dataframe
    print("Fetching FIFA matches dataframe from DB (source='csv_backfill')...")
    df_fifa_raw = load_match_data(engine, "fifa")
    unique_fifa_matches = df_fifa_raw.select("match_id").n_unique()
    print(f"-> FIFA raw query unique match_id count: {unique_fifa_matches:,}")

    # a) FIFA Goals O/U
    print("\n[1/5] Running build_fifa_goals_ou_features()...")
    df_fifa_ou = build_fifa_goals_ou_features(df_fifa_raw)
    ...
    df_fifa_ah = build_fifa_asian_handicap_features(df_fifa_raw)
    ...
    df_fifa_ml = build_fifa_money_line_features(df_fifa_raw)
    ...
    # 2. eBasketball Matches Dataframe
    print("\nFetching eBasketball matches dataframe from DB (source='csv_backfill')...")
    df_ebasket_raw = load_match_data(engine, "ebasket")
    unique_ebasket_matches = df_ebasket_raw.select("match_id").n_unique()
    print(f"-> eBasketball raw query unique match_id count: {unique_ebasket_matches:,}")

    # d) eBasketball O/U
    print("\n[4/5] Running build_ebasket_ou_features()...")
    df_ebasket_ou = build_ebasket_ou_features(df_ebasket_raw)
    ...
    df_ebasket_ml = build_ebasket_money_line_features(df_ebasket_raw)
